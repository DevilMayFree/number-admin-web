...
import { authUserPage } from "@/api/system/user";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      authUserPage(this.queryParams).then(response => {
    ...
    cancelAuthUser(row) {
      ...
      this.$modal.confirm('确认要取消该用户"' + row.username + '"角色吗？').then(function() {
        return authUserCancel({ userIds: delIds, roleId: roleId });
      }).then(() => {
        this.getList();
