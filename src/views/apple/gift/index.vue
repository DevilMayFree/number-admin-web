...
<script>
import {addBatch, getTakeContent, pageAppleGift, pageAppleGiftLog, updateTake,} from "@/api/apple/gift";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      pageAppleGift(this.queryParams).then(response => {
    ...
    getLogList() {
      ...
      pageAppleGiftLog(this.logQueryParams).then(response => {
